...
bot.dialog('/', [(session, args, next) => {
    ...
    store.dispatch(Actions.receiveMessage(text, attachments));
    store.dispatch(Actions.sendMessage('Sorry, I don\'t know what you say. You can say: add.'));
    store.dispatch(Actions.endDialog());
}]);
...
HOISTED_INTENTS.forEach(name => {
    bot.dialog(`/${ name }`, [(session, args, next) => {
        ...
        if (args) {
            store.dispatch(Actions.receiveLuisIntent(args.intent));
        }
    }]).triggerAction({
        matches: `${ name }`
    });
